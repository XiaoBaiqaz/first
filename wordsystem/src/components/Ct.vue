<template>
    ...
    <el-table-column  align="center" header-align="center" label="rate" prop="rate"></el-table-column>
    <!-- <el-table-column  align="center" header-align="center">
      <template slot-scope="sp">{{ sp.row.rate }}</template>
    </el-table-column> -->
  </el-table>
...
export default {
  ...
  mounted:function(){
    axios.get("http://localhost:8081/ct/"+this.pagenum+"/"+this.pagesize).then((res)=>{
  ...
  methods:{
    changeStatue(val){
      /* alert(val) */
      axios.get("http://localhost:8081/ct/"+val).then(this.getInitial);
    },
    getPageNum(val){
